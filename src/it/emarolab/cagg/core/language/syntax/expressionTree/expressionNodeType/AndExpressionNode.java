...
public class AndExpressionNode extends ExpressionNode< ExpressionEmptyData>{
	...
	public AndExpressionNode(){								// empty constructor. ex: A" "B
		super( MAX_CHILDREN_NUMBER);
	}
	public AndExpressionNode( ParserRuleContext ctx){		// nothing to parse on constructor.
		super( MAX_CHILDREN_NUMBER); 
		// care about ctx == null (on copy) for initialisaton
	}
	@Override
	public AndExpressionNode copy( Boolean updateCopyId) {
		return (AndExpressionNode) this.copyContents( new AndExpressionNode( null), updateCopyId);
	}
	...
	protected Type assignType() {
		return Type.AND;		// return node type
	}
	...
	public ExpressionNode<ExpressionEmptyData>.NodeEvaluator getEvaluator() {
		return new NodeEvaluator(){
			...
			public Boolean evaluate(List<Boolean> childrenValue) {
				return childrenValue.get( 0) & childrenValue.get( 1);
			}
		};
	}
